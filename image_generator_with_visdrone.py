...
class VisDroneDataGenerator:
    ...
    def _create_background(self, size: Tuple[int, int] = (800, 800)) -> np.ndarray:
    ...
    def generate_image(self, num_objects: int = 5) -> Tuple[np.ndarray, List[Dict], int]:
        """Генерация изображения с объектами"""
        # Создание фона
        ...
        return background, bboxes, len(bboxes)
...
if __name__ == "__main__":
    generator = VisDroneDataGenerator('/workspaces/2d-graphics/visdrone-dataset')
    
    # Генерация изображения
    img, bboxes, num_objects = generator.generate_image(num_objects=5)
    ...
    axs[0].set_title(f'Количество объектов: {num_objects}')
    axs[0].axis('off')
    ...
    # Гистограмма
    axs[1].hist(img.ravel(), bins=256, color='orange', alpha=0.7)
